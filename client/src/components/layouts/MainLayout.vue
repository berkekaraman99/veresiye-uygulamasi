...
<template>
  <div class="relative">
    ...
    <div
      v-if="width > 639"
      id="SideNav"
      class="fixed z-0 bg-[var(--sidebar-color)] ml-[6px] top-[72px] bottom-2 rounded-xl border-r border-r-[var(--secondary-variant)] shadow-lg"
      ...
      <ul class="w-full" :class="[!openSideNav ? 'p-2' : 'px-2 pb-2 pt-[7px]']">
        <RouterLink v-for="item in navigation" :key="item.name" :to="{ name: item.href }">
          <li
            ...
            :class="[!openSideNav ? ' justify-center ps-4' : 'px-3 ']"
          >
      ...
      <ul class="mt-4 w-full px-3 pb-2 pt-[7px]">
        <RouterLink v-for="item in navigation" :key="item.name" :to="{ name: item.href }">
          <li class="group flex items-center rounded-lg py-2 text-base font-medium hover:bg-[var(--primary)] px-3">
            ...
            <div class="font-semibold text-[var(--text-light)] text-[14px] ms-4">{{ item.name }}</div>
          </li>
        </RouterLink>
      </ul>
    </div>
...
.sidebar-item-text {
  @apply font-semibold text-[var(--text-light)] text-[14px] ms-4 text-nowrap overflow-hidden;
}
</style>
